...
function Header()
{
    ...
    //Fecha
    $this->Cell(40,10,"Fecha:".$arr["Fecha"]."" ,0,0);
...
function TablaServicios()
{
    ...
    $this->Ln(9);
    $header = array('Clave','Diagnostico o Servicio Realizado'
    ...
    foreach($Datos as $row)
    {
        ...
        if($row[4] == 1){
            $this->Cell($w[0],10,$row[2],'LR',0,'L',$fill);
            $this->Cell($w[1],10,$row[3],'LR',0,'L',$fill);
            $this->Cell($w[2],10,$row[0],'LR',0,'L',$fill);
            $this->Cell($w[2],10,$row[5],'LR',0,'L',$fill);
            $this->Cell($w[2],10,$row[6],'LR',0,'L',$fill);
            $this->Cell($w[5],10,$row[1],'LR',0,'L',$fill);
        $this->Ln();
        $fill = !$fill;
        }
    }
...
function TablaMateriales(){
    $this->Ln(9);
    $header = array('Clave','Material Utilizado'
    , 'Cantidad', 'Precio', 'Total',
    'Comentarios');
    // Colores, ancho de línea y fuente en negrita
    $this->SetFillColor(0, 25, 79);
...
function Totales(){
    ...
    //  //Movemos hacia la derecha el siguiente campo
     $this->Cell(150);
    //  //SubTotal
     $this->Cell(28,10,"SubTotal:".$arr["SubTotal"]."" ,0,0,'L'); 
    $this->Ln(6);
    // //Refacciones
    ...
    //Refacciones
     ...
     $this->Cell(150);
     //SubTotal
     $this->Cell(28,10,"Total:".$arr["Total"]."" ,0,0,'L'); 
}
